...
// los días en los que el alumno faltó o no tocó el registro).
export function contarBloquesTranscurridos(bloques, registro) {
  ...
  for (const fecha of fechasActividad) {
    ...
    const diaSemana = fechaDia.getDay() === 0 ? 7 : fechaDia.getDay()
    contador += bloques.filter(b => b.dia_semana === diaSemana).length
  }
  return contador
}
